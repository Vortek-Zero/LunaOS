...
class MetricsAggregator:
    ...
    def _aggregate_traces(self, metrics: dict[str, Any]):
        ...
        if metrics["planner"]["total"] > 0:
            metrics["planner"]["success_rate"] = round(
                metrics["planner"]["success"] / metrics["planner"]["total"] * 100, 1
            )

        if metrics["reflection"]["total"] > 0:
            metrics["reflection"]["success_rate"] = round(
                metrics["reflection"]["success"] / metrics["reflection"]["total"] * 100, 1
            )
