...
class Memory:
        ...
        with self._data_lock:
            try:
                ...
                self.sessions[session_id] = [
                    {"role": m["role"], "text": m["text"], "ts": m.get("ts", "")} for m in rows
                ]
            except Exception:
    ...
    def add_exchange(self, user_text: str, assistant_text: str) -> None:
        with self._data_lock:
            self.history.append({"role": "user", "text": user_text, "ts": datetime.now().isoformat()})
            self.history.append({"role": "assistant", "text": assistant_text, "ts": datetime.now().isoformat()})
    ...
    def remember(self, fact: str, category: str = "geral", importance: float = 0.5) -> None:
        ...
        with self._data_lock:
            ...
            if len(self.facts) > MAX_PERSISTENT:
                self.facts = sorted(self.facts, key=lambda f: f.get("importance", 0), reverse=True)[:MAX_PERSISTENT]
                self._rebuild_index()
    ...
    def recall_critical(self) -> list[str]:
        ...
        """
        with self._data_lock:
            return [f["fact"] for f in self.facts if f.get("importance", 0) >= 0.85]
        ...
        with self._data_lock:
            ...
            if not topic_change:
                ...
                for cat in relevant_categories:
                    cat_facts = [f for f in self.recall_by_category(cat, limit=5) if f not in already_included]
                    if cat_facts:
    ...
    def _schedule_save(self) -> None:
        ...
        with self._save_lock:
            ...
            self._save_timer = threading.Timer(MEMORY_SAVE_DEBOUNCE_SECONDS, self._save)
    ...
    def _save(self) -> None:
        with self._data_lock:
            ...
            try:
                ...
                MEMORY_FILE.write_text(json.dumps(data_to_save, ensure_ascii=False, indent=2), encoding="utf-8")
                self._dirty = False
            except Exception as e:
