...
def generate_dashboard() -> Layout:
    ...
    layout["left"].split_column(Layout(name="cognition", size=10), Layout(name="memory"))
    layout["right"].split_column(Layout(name="tools", size=10), Layout(name="performance"))

    # HEADER
    layout["header"].update(
        Panel("🌙 [bold magenta]LUNA DEBUG DASHBOARD[/] (v1.4.1 Stabilization)", style="white on blue")
    )
    ...
    cog_table.add_row(
        "Planner Success",
        f"[bold green]✓ {planner.get('success_rate', 0)}%[/] ({planner.get('success')}/{planner.get('total')})",
    )

    reflection = metrics.get("reflection", {})
    cog_table.add_row(
        "Reflection Efficiency",
        f"[bold green]✓ {reflection.get('success_rate', 0)}%[/] ({reflection.get('success')}/{reflection.get('total')})",
    )
