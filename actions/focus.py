...
class FocusManager:
    def __init__(self):
        self._session: dict | None = None  # {type, duration, end_time, thread, cancelled}
    ...
    def _save_session(self, label: str, minutes: int, status: str) -> None:
        ...
        try:
            ...
            FOCUS_LOG_FILE.write_text(json.dumps(self._log, ensure_ascii=False, indent=2), encoding="utf-8")
        except Exception:
            pass
    ...
    def handle(self, text: str) -> str | None:
        ...
        # Foco / Pomodoro
        if any(
            w in tl for w in ["modo foco", "pomodoro", "iniciar foco", "começar foco", "sessão de foco", "foco por"]
        ):
            m = re.search(r"(\d+)\s*(?:minutos?|min)", tl)
