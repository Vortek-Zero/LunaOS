...
class ClipboardManager:
    def __init__(self):
        ...
        tool = (
            ...
            else ("xclip" if self._has_xclip else "xsel" if self._has_xsel else "nenhuma")
        )
    ...
    def write(self, text: str) -> str:
        ...
        try:
            ...
            if self._has_xclip:
                r = subprocess.run(["xclip", "-selection", "clipboard"], input=text, text=True, timeout=3)
                if r.returncode == 0:
                    self._history.append(text[:100])
                    return f"📋 Copiado: '{text[:60]}{'...' if len(text) > 60 else ''}'"
            if self._has_xsel:
                r = subprocess.run(["xsel", "--clipboard", "--input"], input=text, text=True, timeout=3)
                if r.returncode == 0:
                    self._history.append(text[:100])
                    return "📋 Copiado."
...
# Singleton
_clipboard_instance: ClipboardManager | None = None
